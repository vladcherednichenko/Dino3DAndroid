...
import com.vlad.dino3d.min3d.Utils;
...
import static com.vlad.dino3d.min3d.Min3d.TAG;
...
public class ObjParser extends AParser implements IParser {
		...
		try {
			while ((line = buffer.readLine()) != null) {
				...
				if (type.equals(VERTEX)) {
					...
					if(firstObject)
					{
						Log.d(TAG, "Create object " + objName);
						co.name = objName;
						firstObject = false;
					}
					else
	...
	public Object3dContainer getParsedObject() {
		Log.d(TAG, "Start object creation");
		...
		for (int i = 0; i < numObjects; i++) {
			...
			Log.d(TAG, "Creating object " + o.name);
		...
		Log.d(TAG, "Object creation finished");
